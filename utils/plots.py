...
def plot_single_image(image, target, path=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    # Plot image grid with labels
    ...
    _, h, w = image.shape  # batch size, _, height, width
    ...
    for i in range(i + 1):
        ...
        annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        if path:
            annotator.text((x + 5, y + 5 + h), text=Path(path).name[:40], txt_color=(220, 220, 220))  # filenames
        if len(target) > 0:
            ti = target[target[:, 0] == i]  # image targets
            boxes = xywh2xyxy(ti[:, 2:6]).T
            classes = ti[:, 1].astype('int')
            labels = ti.shape[1] == 6  # labels if no conf column
            conf = None if labels else ti[:, 6]  # check for confidence presence (label vs pred)
